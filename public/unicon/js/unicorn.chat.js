$(document).ready(function(){
	...
	function initial_socket(){
		...
		socket.on('to_client',function(data){
			add_message(data.from,'/unicon/img/demo/av1.jpg',data.msg,true);
		});
	...
	function send_msg(from,to,msg){
		...
		mdata.to = to;
		mdata.msg = msg;
		//var mm = JSON.stringify(mdata);
		socket.emit('chat_msg',mdata);
	}
	...
	$('.chat-message button').click(function(){
		...
		if(input.val() != ''){
			send_msg(uid,to_uid,input.val());
			add_message(uid,'/unicon/img/demo/av1.jpg',input.val(),true);
		}		
	});
	...
	$('.chat-message input').keypress(function(e){
		if(e.which == 13) {	
			if($(this).val() != ''){
				send_msg(uid,to_uid,$(this).val());
				add_message(uid,'/unicon/img/demo/av1.jpg',$(this).val(),true);
			}		
		}
	});
